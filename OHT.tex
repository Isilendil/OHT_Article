...
Transfer learning, which devotes to transferring knowledge extracted from the source domain to the target domain, has been actively studied in recent years.
...
A great number of works in the last decade successfully deal with homogeneous transfer learning problem, which assumes that the data spaces of the source and target domains are the same.
...
The topics that our work is related to are traditional transfer learning, heterogeneous transfer learning and online learning.
In this section, we review some related works.

\subsection{Traditional Transfer Learning}
The objective of traditional transfer learning is to solve a problem in the target domain by leveraging knowledge extracted from some source domains.
The task in the target domain includes classification, regression, clustering, recommendation and so forth.
Two of the most frequently used strategies are instance transfer and feature transfer.
Instance transfer repeatly increases the weight of the "good" instances in the source domain, and decreases the weight of the "bad" ones.
Feature transfer usually constructs common feature representation of different domains.

\subsection{Heterogeneous Transfer Learning}
According to the relationship among data space of different domains, we can categorize transfer learning into two groups: homogeneous transfer and heterogeneous transfer.
Homogeneous transfer considers a problem that the data spaces of source and target domains are the same.
While heterogeneous transfer solve the problem that the data spaces of source and target domains are different.
cite proposed a algorithm to utilize data with different output space.
cite leveraged text data for image clustering, yet cite utilized text data for image classification task.
cite proposed a algorithm to build two separate classifiers based on text features and image features, respectively.
And then the final classifier are trained to combine them.
cite constructs transition probabilities based on co-occurrence information among instances from different domains, and applied random walker method to deal with classification task.
...
One of the classic algorithms of online learning is Perceptron algorithm, which move the decision bound towards the direction of an instance which is labeled wrong.
Passive-Aggressive algorithm and its variants take the criterion of maximum margin into account.

However, few works address transfer learning in an online fasion.
cite investigated online transfer learning, which aims to solve an online learning problem in the target domain using training data in the source domain.
cite adopt an ensemble strategy to deal with the homogeneous transfer learning problem, and a co-regularization principle is used to handle the heterogeneous transfer learning problem.
...
In order to transfer knowledge in the heterogneous source to the target domain, we construct connection between two domains, and then design a hypothesis to predict class label of the instance in the target domain.
...
$$ h_{t}^{s}(\mathbf{x}_t) = \sum\limits_{\mathbf{x}_i \in N} \frac{Sim_t(i)}{\sum\limits_{\mathbf{x}_t \in N} Sim_t(i)} y_i $$
where $N$ is the set consisting of $K$ nearest neighbors in the heterogeneous source domain.
...
The technique of the proof can be found in cite.
\begin{rem}
